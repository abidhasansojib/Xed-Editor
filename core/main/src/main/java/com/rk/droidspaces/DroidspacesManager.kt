...
object DroidspacesManager {
    ...
    suspend fun listContainers(): List<String> =
        withContext(Dispatchers.IO) {
            ...
            // Scan container directories exactly like Droidspaces app
            val result = DroidspacesShell.runCommand(
            ...
            if (result.isSuccess) {
                for (line in result.outLines) {
                    val dir = line.trim().removeSuffix("/")
                    val sanitizedName = dir.substringAfterLast("/")
                    if (sanitizedName.isNotEmpty()) {
                        // Check container.config for real name if present
                        val configCheck = DroidspacesShell.runCommand(
                            "grep '^name=' '$dir/container.config' 2>/dev/null",
                            timeoutMs = 3000,
                        )
                        val realName = configCheck.stdout.lines()
                            .firstOrNull { it.startsWith("name=") }
                            ?.removePrefix("name=")
                            ?.trim()

                        containers.add(if (!realName.isNullOrBlank()) realName else sanitizedName)
                    }
                }
            }
    ...
    suspend fun getContainerStatus(containerName: String): ContainerStatus =
        withContext(Dispatchers.IO) {
            ...
            // Droidspaces CLI pid command: returns PID or "NONE"
            ...
            val sanitizedName = containerName.replace(" ", "-").replace("'", "'\\''")
            val exists = DroidspacesShell.runCommand(
                "test -d '${DroidspacesConstants.CONTAINERS_BASE_PATH}/$escapedName' || test -d '${DroidspacesConstants.CONTAINERS_BASE_PATH}/$sanitizedName' && echo 1 || echo 0",
                timeoutMs = 4000,
            )
        ...
        withContext(Dispatchers.IO) {
            ...
            if (!passwdResult.isSuccess || passwdResult.stdout.isBlank()) {
                passwdResult = DroidspacesShell.runCommand(
                    "cat '${DroidspacesConstants.CONTAINERS_BASE_PATH}/$escapedName/rootfs/etc/passwd' 2>/dev/null || cat '${DroidspacesConstants.CONTAINERS_BASE_PATH}/$sanitizedName/rootfs/etc/passwd' 2>/dev/null",
                    timeoutMs = 5000,
                )
            }
