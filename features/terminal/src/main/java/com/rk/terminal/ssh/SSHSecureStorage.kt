 ...
 *
 * Credentials are encrypted before writing to SharedPreferences and decrypted
 * in-memory only when establishing an SSH connection. Plaintext credentials are
 * never stored in standard Settings preferences or leaked in logs.
 */
object SSHSecureStorage {
    ...
    private fun encrypt(plaintext: String): String? {
        ...
        return try {
            ...
            // Fallback for non-standard environments (e.g. unit tests without Keystore provider)
    ...
    fun getKeyPassphrase(): String = decrypt(prefs?.getString(KEY_SSH_KEY_PASSPHRASE, null))
