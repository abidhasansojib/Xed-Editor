...
class SSHTerminalBridge(
    ...
    var sessionClient: TerminalSessionClient?,
    ...
    fun write(data: ByteArray, offset: Int = 0, count: Int = data.size) {
        if (!isConnected && active.get().not()) {
            // Auto reconnect on key press if disconnected
            start(session.emulator?.mColumns ?: 80, session.emulator?.mRows ?: 24)
            return
        }
